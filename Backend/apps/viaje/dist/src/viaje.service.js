...
let ViajeService = class ViajeService {
    ...
    async buscarUnidadesDisponibles(fechaInicio, dtoViaje) {
        console.log(dtoViaje);
        const { fecha, hora } = await this.calcularFechaRegreso(dtoViaje.dto.origenCoords, dtoViaje.dto.destinoCoords, constantesTiempoViaje_1.BASE_COORDS, fechaInicio, constantesTiempoViaje_1.TIEMPO_MUERTO);
        const viajesEnRango = await this.viajeRepository.find({
            where: [
                { fechaInicio: (0, typeorm_2.LessThanOrEqual)(new Date(fecha)), fechaFin: (0, typeorm_2.MoreThanOrEqual)(fechaInicio) },
            ]
        });
    ...
    async confirmarPagoViajeSenia(viajeId) {
    ...
    async confirmarPagoViajeResto(viajeId) {
        await this.viajeRepository.update(viajeId, {
            estadoViaje: { id: 5 }
        });
