...
let AuthGuard = class AuthGuard {
    ...
    async canActivate(context) {
        ...
        if (!token) {
            throw new common_1.UnauthorizedException(); // excepcion si no hay token
        }
        try {
            const payload = await this.jwtService.verifyAsync(token, {
                secret: jwt_constant_1.jwtConstants.secret, // usa la clave secreta definida en jwtConstants.secret
            });
            request.user = payload; // si es válido, guarda el payload decodificado aca
        }
        catch (error) {
            throw new common_1.UnauthorizedException(); // sino lanza excepcion
        }
        return true;
    }
