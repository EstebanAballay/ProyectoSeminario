...
const empresa_data_1 = require("./empresa.data");
...
let BillingService = class BillingService {
    constructor(cobroRepo, abonanteRepo) {
        this.cobroRepo = cobroRepo;
        this.abonanteRepo = abonanteRepo;
    }
    async generarFactura(cobroId, viajeId) {
        ...
        if (!cobro || !viaje)
            throw new common_1.NotFoundException('Cobro o Viaje no encontrado');
        ...
        <body>
            <div class="header-container">
                <div class="company-info">
                    <div class="company-name">${empresa_data_1.EMPRESA_DATA.nombre}</div>
                    <div>Dirección: ${empresa_data_1.EMPRESA_DATA.direccion}</div>
                    <div>Tel: ${empresa_data_1.EMPRESA_DATA.telefono} | Email: ${empresa_data_1.EMPRESA_DATA.email}</div>
                    <div class="bold" style="margin-top: 5px;">I.V.A.: ${empresa_data_1.EMPRESA_DATA.condicionIva}</div>
                </div>
                ...
                <div class="invoice-info">
                    ...
                    <div>CUIT: ${empresa_data_1.EMPRESA_DATA.cuit}</div>
                    <div>IIBB: ${empresa_data_1.EMPRESA_DATA.ingresosBrutos}</div>
                    <div>Inicio Actividad: ${empresa_data_1.EMPRESA_DATA.inicioActividad}</div>
                </div>
            </div>
    ...
    async getViaje(id) {
        const viaje = await axios_1.default.get(`http://viaje-service:3004/viaje/viaje-con-unidades/${id}`);
        return viaje.data;
    }
};
...
exports.BillingService = BillingService = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(abonante_entity_1.Abonante)),
